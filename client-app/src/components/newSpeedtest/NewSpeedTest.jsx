...
const NewSpeedTest = () => {
  ...
  const [testStage, setTestStage] = useState(null);
  ...
  useEffect(() => {
    if (testStage === "download") {
      let timeElapsed = 0;
      let initialDownload = parseFloat((Math.random() * 20 + 10).toFixed(2));
  ...
  const TypographyBox = ({ label, value, variantLabel, variantValue }) => (
    <Box sx={{ display: "flex", justifyContent: "space-between" }}>
      <Typography variant={variantLabel} color="#676767">
        {label}
      </Typography>
